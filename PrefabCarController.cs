...
public class PrefabCarController : MonoBehaviour {
	...
	private void IsSkidding(){
		float _skidfactor = .7f; //lower number, easier skid
		...
		else if (Speed > 6 && m_verticalInput < -.5f){
			Skid();
		}
		else if (Speed > 10 && m_horizontalInput > .8f || Speed > 6 && m_verticalInput < -.8f){
			Skid();
        }
